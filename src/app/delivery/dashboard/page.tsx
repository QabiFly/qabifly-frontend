...
export default function DeliveryDashboard() {
  ...
  const { user, ready } = useGuard("DELIVERY_BOY");
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (e.response?.status !== 404) {
        console.error("Delivery load error:", e.response?.status);
      }
  ...
  const accept = async (num: string) => {
    try {
      await api.post(`/orders/${num}/delivery/accept/`);
      toast.success("✅ Delivery accept! Pickup ke liye jao.");
      loadAll();
    } catch (e: any) {
  ...
  const setDigit = (v: string, i: number) => {
    ...
    if (a.join("").length === 6) setTimeout(() => verifyOTP(a.join("")), 100);
  };
  ...
  if (!ready || (loading && !deliveries.length)) return (
      ...
      <div className="max-w-md mx-auto px-4 pt-4 pb-24">
        ...
        <div className="grad rounded-3xl p-5 mb-5 text-white shadow-xl shadow-purple-300/30">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-white/60 text-xs uppercase tracking-wide">Delivery Boy</p>
              <h1 className="font-extrabold text-xl mt-0.5">{user?.virtual_name || user?.full_name}</h1>
              {user?.virtual_number && (
        ...
        <button onClick={() => router.push("/wallet")}
          ...
          <div>
            <p className="font-bold text-sm text-gray-900">Meri Earnings</p>
        ...
        {active.length > 0 && (
          <div className="mb-4">
            ...
            {active.map((d:any) => (
              <div key={d.id} className="bg-white rounded-2xl p-4 shadow-sm border-l-4 border-orange-400 mb-3">
                <div className="flex justify-between mb-2">
                  <span className="font-mono font-bold text-sm">#{d.order_number}</span>
                  <span className="font-extrabold text-green-600">+{formatRupee(d.delivery_charge||30)}</span>
                </div>
                ...
                <p className="text-xs text-gray-400 mb-3">{d.total_items} items · {timeAgo(d.created_at)}</p>
                <div className="grid grid-cols-2 gap-2">
                  <a href={`https://maps.google.com/?q=${encodeURIComponent(d.delivery_address||"")}`}
                    ...
                    <Navigation size={13} /> Map Open
                  </a>
                  <button onClick={() => { setOtpModal(d.order_number); setOtp(["","","","","",""]); setTimeout(()=>document.getElementById("dotp0")?.focus(),200); }}
        ...
        {pending.length === 0 ? (
          <div className="bg-white rounded-2xl p-10 text-center shadow-sm mb-4">
            ...
            <p className="text-xs text-gray-300 mt-1">Auto-refresh ho raha hai</p>
          </div>
        ) : (
            ...
            {pending.map((d:any) => (
              <div key={d.id} className="bg-white rounded-2xl p-4 shadow-sm">
                ...
                <div className="flex items-center gap-3 text-xs text-gray-400 mb-3">
                  ...
                  <span><Clock size={10} className="inline mr-0.5" />{timeAgo(d.created_at)}</span>
                </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {completed.slice(0,5).map((d:any) => (
                <div key={d.id} className="bg-white rounded-xl px-4 py-3 shadow-sm flex items-center justify-between">
                  <div>
          ...
          <div className="bg-white w-full rounded-t-3xl p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-gray-400 text-sm">Order <span className="font-mono font-bold text-purple-600">#{otpModal}</span></p>
              </div>
            ...
            <p className="text-sm text-gray-600 mb-4 bg-blue-50 rounded-xl p-3">
              📱 Customer ke phone pe OTP aaya hoga. Unse maangein.
            </p>
            <div className="flex gap-2 mb-5">
              {otp.map((d, i) => (
                <input key={i} id={`dotp${i}`}
                  type="text" inputMode="numeric" maxLength={1} value={d}
                  onChange={e => setDigit(e.target.value, i)}
                  onKeyDown={e => { if (e.key==="Backspace" && !d && i>0) document.getElementById(`dotp${i-1}`)?.focus(); }}
            ...
            <div className="flex gap-3">
              <button onClick={() => {setOtpModal(null); setOtp(["","","","","",""]);}}
                className="flex-1 py-3.5 bg-gray-100 text-gray-600 font-bold rounded-xl">Cancel</button>
