...
export default function OnboardingPage() {
  ...
  const submit = async () => {
    if (!vname.trim()) {
      toast.error("Enter name for your Virtual number");
      return;
    }
      ...
      <div className="max-w-sm mx-auto px-4 py-6">
        ...
        {step === 1 && (
          <div>
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs font-semibold text-gray-600 block mb-1">Village / Town *</label>
                <input
                  value={village}
                  onChange={(e) => setVillage(e.target.value)}
                  placeholder="e.g. Reoti"
            ...
            <Btn
              ...
              onClick={() => {
                if (!village.trim() || !district.trim()) {
                  toast.error("Village and district are required");
                  return;
                }
                setStep(2);
              }}
        ...
        {step === 2 && (
          <div>
            ...
            <p className="text-xs text-gray-400 mb-3">
              Virtual number format: <span className="font-mono font-bold text-purple-600">@ROI007xx</span>
            </p>

            {sc && (
              <div className="bg-purple-50 border border-purple-200 rounded-xl px-4 py-2.5 text-center mb-3">
                <p className="text-xs text-purple-500 font-semibold">Your number will be:</p>
                <p className="text-lg font-extrabold text-purple-700 font-mono">@{sc}XXXXX</p>
              </div>
            )}
            ...
            <div className="max-h-64 overflow-y-auto space-y-1.5 scrollbar-hide">
              {filtered.length === 0 && (
                <p className="text-center text-gray-400 text-sm py-4">No station found</p>
              )}
              {filtered.map((c) => (
                <button
                  key={c.code}
                  onClick={() => setSc(c.code)}
                ...
                >
                  <div>
                    <span className="font-extrabold text-gray-900 font-mono text-sm">@{c.code}</span>
                    <span className="text-gray-500 text-sm ml-2">{c.location}</span>
                  </div>
                  {sc === c.code && <Check size={16} className="text-purple-600 flex-shrink-0" />}
                </button>
              ))}
            </div>
        ...
        {step === 3 && (
          <div>
            ...
            <p className="text-xs text-gray-400 mb-5">This name will be visible publicly</p>
            ...
            <div className="mb-5">
              <label className="text-xs font-semibold text-gray-600 block mb-1">Virtual Name *</label>
              <input
                value={vname}
            ...
            <div className="flex gap-3">
              ...
              <Btn onClick={submit} loading={load} className="flex-1">Complete Setup ✓</Btn>
            </div>
          </div>
        )}
      </div>
